...
@app.route('/api/orders/<order_id>/manual-fill', methods=['POST'])
def manual_fill_order(order_id):
    """
    Manually mark an order as filled and place profit/trailing stop order.
    Used when E*TRADE API is unavailable but user knows order filled.
    """
    try:
        data = request.get_json()
        ...
        if not fill_price:
            return jsonify({'success': False, 'error': 'fill_price is required'}), 400

        logger.info(f"Manual fill for order_id={order_id} at price {fill_price}")
        ...
        if matching_key:
            # This is a profit target order
            profit_order = _pending_profit_orders[matching_key]

            if profit_order['status'] != 'waiting':
                return jsonify({
                    'success': False,
                    'error': f"Profit order status is {profit_order['status']}, not waiting"
                })

            # Calculate profit price
            if profit_order['profit_offset_type'] == 'dollar':
                if profit_order['opening_side'] in ['BUY', 'BUY_TO_COVER']:
                    profit_price = fill_price + profit_order['profit_offset']
                else:
                    profit_price = fill_price - profit_order['profit_offset']
            else:  # percent
                if profit_order['opening_side'] in ['BUY', 'BUY_TO_COVER']:
                    profit_price = fill_price * (1 + profit_order['profit_offset'] / 100)
                else:
                    profit_price = fill_price * (1 - profit_order['profit_offset'] / 100)
            ...
            client = _get_authenticated_client()
            result = client.place_order(
                profit_order['account_id_key'],
                {
                    'symbol': profit_order['symbol'],
                    'orderAction': 'SELL' if profit_order['opening_side'] in ['BUY', 'BUY_TO_COVER'] else 'BUY_TO_COVER',
                    'quantity': profit_order['quantity'],
            ...
            # Update status
            profit_order['status'] = 'placed'
            profit_order['fill_price'] = fill_price
            profit_order['profit_price'] = profit_price
            profit_order['profit_order_id'] = result.get('order_id')

            logger.info(f"Manual profit order placed: {result.get('order_id')} at {profit_price}")
            ...
            return jsonify({
                ...
                'profit_order_id': result.get('order_id'),
                'type': 'profit_target'
            })

        # Check if this is a trailing stop order
        trailing_stop_manager = get_trailing_stop_manager()
        ts = trailing_stop_manager.get_trailing_stop(int(order_id))

        if ts:
            # This is a trailing stop order
            trailing_stop_manager.mark_filled(int(order_id), fill_price)

            logger.info(f"Manual trailing stop marked filled at {fill_price}")
            ...
            return jsonify({
                ...
                'type': 'trailing_stop',
                'message': f"Marked filled at {fill_price}. Waiting for price to reach {ts.trigger_price}"
            })

        return jsonify({
            'success': False,
            'error': f'No pending profit target or trailing stop found for order {order_id}'
        }), 404

    except Exception as e:
        logger.error(f"Manual fill failed: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
